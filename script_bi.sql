...
IF OBJECT_ID('BI_SYSTEAM.BI_ESTADO_ANUNCIO', 'U') IS NOT NULL
    DROP TABLE BI_SYSTEAM.BI_ESTADO_ANUNCIO
GO

IF OBJECT_ID('BI_SYSTEAM.BI_ESTADO_ALQUILER', 'U') IS NOT NULL
    DROP TABLE BI_SYSTEAM.BI_ESTADO_ALQUILER
GO
...
CREATE TABLE BI_SYSTEAM.BI_ESTADO_ANUNCIO (
	...
	ESTADO_ANUNCIO_DESCRIPCION NVARCHAR(255)
)

CREATE TABLE BI_SYSTEAM.BI_ESTADO_ALQUILER (
	ID_ESTADO_ALQUILER NUMERIC(18,0) IDENTITY(1,1) PRIMARY KEY,
	ESTADO_ALQUILER_DESCRIPCION NVARCHAR(255)
)
...
CREATE TABLE BI_SYSTEAM.BI_FACT_ANUNCIO (
	...
	ID_ESTADO_ANUNCIO NUMERIC(18,0),
...
ALTER TABLE BI_SYSTEAM.BI_FACT_ANUNCIO
ADD CONSTRAINT FK_ESTADO_ANUNCIO_BI
    FOREIGN KEY (ID_ESTADO_ANUNCIO) 
    REFERENCES BI_SYSTEAM.BI_ESTADO_ANUNCIO(ID_ESTADO_ANUNCIO);
...
ALTER TABLE BI_SYSTEAM.BI_FACT_ALQUILER
ADD CONSTRAINT FK_ESTADO_ALQUILER_BI
    FOREIGN KEY (ID_ESTADO_ALQUILER) 
    REFERENCES BI_SYSTEAM.BI_ESTADO_ALQUILER(ID_ESTADO_ALQUILER);
...
IF OBJECT_ID('BI_SYSTEAM.CALCULAR_AÑO', 'FN') IS NOT NULL
    DROP FUNCTION BI_SYSTEAM.CALCULAR_AÑO;
GO

IF OBJECT_ID('BI_SYSTEAM.CALCULAR_MES', 'FN') IS NOT NULL
    DROP FUNCTION BI_SYSTEAM.CALCULAR_MES;
GO

IF OBJECT_ID('BI_SYSTEAM.CALCULAR_TIEMPO_ID', 'FN') IS NOT NULL
    DROP FUNCTION BI_SYSTEAM.CALCULAR_TIEMPO_ID;
GO

IF OBJECT_ID('BI_SYSTEAM.CALCULAR_RANGO_ETARIO_AGENTE', 'FN') IS NOT NULL
    DROP FUNCTION BI_SYSTEAM.CALCULAR_RANGO_ETARIO_AGENTE;
GO

IF OBJECT_ID('BI_SYSTEAM.CALCULAR_RANGO_ETARIO_INQUILINO', 'FN') IS NOT NULL
    DROP FUNCTION BI_SYSTEAM.CALCULAR_RANGO_ETARIO_INQUILINO;
GO

IF OBJECT_ID('BI_SYSTEAM.CALCULAR_RANGO_METROS', 'FN') IS NOT NULL
    DROP FUNCTION BI_SYSTEAM.CALCULAR_RANGO_METROS;
GO
...
RETURN (
	...
	WHERE 
	(T.TIEMPO_ANIO = BI_SYSTEAM.CALCULAR_AÑO(@id) 
	AND T.TIEMPO_MES = (BI_SYSTEAM.CALCULAR_MES(@id)-1))

	OR 

	(T.TIEMPO_ANIO = (BI_SYSTEAM.CALCULAR_AÑO(@id)-1) 
	AND T.TIEMPO_MES = 12)


	ORDER BY T.TIEMPO_ANIO, T.TIEMPO_MES DESC
	)
END
GO


CREATE FUNCTION BI_SYSTEAM.CALCULAR_RANGO_ETARIO_AGENTE(@AGENTE INT)
RETURNS INT
BEGIN
...
CREATE FUNCTION BI_SYSTEAM.CALCULAR_RANGO_ETARIO_INQUILINO(@INQUILINO INT)
RETURNS INT
BEGIN
	DECLARE @FECHA_NACIMIENTO SMALLDATETIME
	DECLARE @EDAD INT
...
CREATE FUNCTION BI_SYSTEAM.CALCULAR_RANGO_METROS(@INMUEBLE INT)
RETURNS INT
BEGIN
...
IF OBJECT_ID ('BI_SYSTEAM.CARGAR_BI_ESTADO_ANUNCIO') IS NOT NULL
	DROP PROCEDURE BI_SYSTEAM.CARGAR_BI_ESTADO_ANUNCIO
GO

IF OBJECT_ID ('BI_SYSTEAM.CARGAR_BI_ESTADO_ALQUILER') IS NOT NULL
	DROP PROCEDURE BI_SYSTEAM.CARGAR_BI_ESTADO_ALQUILER
GO
...
BEGIN
	...
	SELECT DISTINCT
		ANUNCIO_ESTADO
	FROM SYSTEAM.ANUNCIO
END
GO

--MIGRACION DE BI_ESTADO_ANUNCIO
CREATE PROCEDURE BI_SYSTEAM.CARGAR_BI_ESTADO_ALQUILER
AS
BEGIN
	INSERT INTO BI_SYSTEAM.BI_ESTADO_ALQUILER(
		ESTADO_ALQUILER_DESCRIPCION
	)
	SELECT DISTINCT
		ALQUILER_ESTADO
	FROM SYSTEAM.ALQUILER
END
GO
...
SELECT DISTINCT
   ...
   biea.ID_ESTADO_ANUNCIO,			-- ESTADO ANUNCIO
...
FROM SYSTEAM.ANUNCIO a
	...
	JOIN BI_SYSTEAM.BI_RANGO_M2 birm2 ON birm2.ID_RANGO_M2 = BI_SYSTEAM.CALCULAR_RANGO_METROS(a.INMUEBLE_CODIGO)
	...
	JOIN BI_SYSTEAM.BI_RANGO_ETARIO re ON re.ID_RANGO_ETARIO = BI_SYSTEAM.CALCULAR_RANGO_ETARIO_AGENTE(a.AGENTE_CODIGO)
	JOIN BI_SYSTEAM.BI_ESTADO_ANUNCIO biea ON a.ANUNCIO_ESTADO = biea.ESTADO_ANUNCIO_DESCRIPCION
GROUP BY
   biti.ID_TIPO_INMUEBLE ,	--TIPO INMUEBLES
   birm2.ID_RANGO_M2,		--RANGO METROS CUADRADOS
   bitop.ID_TIPO_OPERACION, --TIPO OPERACION
...
SELECT DISTINCT
	...
	estA.ID_ESTADO_ALQUILER,    --ID_ESTADO_ALQUILER

	sum(isnull(pa.PAGO_ALQUILER_IMPORTE,0)) AS [SUMA PAGO ALQUILER],
	sum(isnull(a.ALQUILER_COMISION,0))		AS [SUMA COMICIONES],
	...
	COUNT(NULLIF(CASE WHEN pa.PAGO_ALQUILER_FECHA >= pa.PAGO_ALQUILER_FECHA_VENCIMIENTO THEN a.ALQUILER_CODIGO END, 0)) AS [PAGOS ATRASADOS],
	COUNT(DISTINCT PA.ALQUILER_CODIGO)                                                             --CANT DE PAGOS
	

	--sum(CASE WHEN pa.PAGO_ALQUILER_FECHA < pa.PAGO_ALQUILER_FECHA_VENCIMIENTO THEN 1 ELSE 0 END)  --PAGO EN TERMINO
	--COUNT(NULLIF(CASE WHEN pa.PAGO_ALQUILER_FECHA >= pa.PAGO_ALQUILER_FECHA_VENCIMIENTO THEN a.ALQUILER_CODIGO END, 0)) AS [PAGOS ATRASADOS],
	--COUNT(NULLIF(CASE WHEN pa.PAGO_ALQUILER_FECHA < pa.PAGO_ALQUILER_FECHA_VENCIMIENTO THEN a.ALQUILER_CODIGO END, 0)) AS [PAGOS EN TERMINO]
...
JOIN BI_SYSTEAM.BI_RANGO_ETARIO re ON re.ID_RANGO_ETARIO = BI_SYSTEAM.CALCULAR_RANGO_ETARIO_INQUILINO(A.INQUILINO_CODIGO)
...
JOIN BI_SYSTEAM.BI_TIEMPO T6 ON T6.TIEMPO_ANIO = year(pa.PAGO_ALQUILER_FECHA_VENCIMIENTO)  AND T6.TIEMPO_MES = month(pa.PAGO_ALQUILER_FECHA_VENCIMIENTO)
...
JOIN BI_SYSTEAM.BI_ESTADO_ALQUILER estA ON estA.ESTADO_ALQUILER_DESCRIPCION = a.ALQUILER_ESTADO
...
SELECT
	...
	SUM(isnull(v.VENTA_PRECIO_VENTA,0))			AS [Suma Ventas],
	SUM(isnull(v.VENTA_COMISION,0))				AS [Suma Comisiones Ventas],
	COUNT(*)									AS [Cantidad Ventas],
	SUM(isnull(i.INMUEBLE_SUPERFICIETOTAL,0))	AS [Suma M2]
...
EXEC BI_SYSTEAM.CARGAR_BI_ESTADO_ALQUILER
...
CREATE VIEW BI_SYSTEAM.VISTA_PORCENTAJE_INCUMPLIMIENTO_PAGO_ALQUILER AS
	SELECT
		...
		SUM(A.ALQUILER_PAGOS)															AS [Total Pagos ],
		SUM(A.ALQUILER_PAGO_ATRASADO)													AS [Pagos atrasados],
		CAST(SUM(A.ALQUILER_PAGO_ATRASADO) AS DECIMAL) / SUM(A.ALQUILER_PAGOS) * 100	AS [Porcentaje incumplimiento]
...
CREATE VIEW BI_SYSTEAM.VISTA_PORCENTAJE_PROMEDIO_INCREMENTO_VALOR_ALQUILERES AS
	...
	FROM BI_SYSTEAM.BI_FACT_ALQUILER al
		...
		JOIN BI_SYSTEAM.BI_FACT_ALQUILER al2 on al2.ID_TIEMPO_FECHA_INICIO = T2.ID_TIEMPO
		JOIN BI_SYSTEAM.BI_ESTADO_ALQUILER estA on estA.ID_ESTADO_ALQUILER= al.ID_ESTADO_ALQUILER
		JOIN BI_SYSTEAM.BI_ESTADO_ALQUILER estA2 on estA2.ID_ESTADO_ALQUILER= al2.ID_ESTADO_ALQUILER
	WHERE estA.ESTADO_ALQUILER_DESCRIPCION = 'Activo' AND estA2.ESTADO_ALQUILER_DESCRIPCION = 'Activo' 
	GROUP BY 
		T1.TIEMPO_ANIO,
		T1.TIEMPO_MES
...
CREATE VIEW BI_SYSTEAM.VISTA_PRECIO_PROMEDIO_M2_VENTA AS
	SELECT
		...
		AVG(v.PRECIO_VENTA / v.CANT_M2)									AS [Precio promedio M2]
		--sum(v.PRECIO_VENTA) / sum(v.CANT_M2)							AS [Precio promedio M2]
	...
	CONCAT(
		...
		convert
			(float,
				...
				JOIN BI_SYSTEAM.BI_ESTADO_ANUNCIO estA ON a2.ID_ESTADO_ANUNCIO = estA.ID_ESTADO_ANUNCIO
				WHERE T2.TIEMPO_ANIO = T1.TIEMPO_ANIO 
				AND estA.ESTADO_ANUNCIO_DESCRIPCION IN ('Alquilado','Vendido')
				)
			) 
		) * 100, '%')
...
CREATE VIEW BI_SYSTEAM.VISTA_MONTO_TOTAL_CIERRE_CONTRATOS AS
  ...
  BI_SYSTEAM.BI_FACT_ANUNCIO a
	...
	JOIN BI_SYSTEAM.BI_TIPO_OPERACION tipoOp ON a.id_tipo_operacion = tipoOp.id_tipo_operacion
	JOIN BI_SYSTEAM.BI_TIPO_MONEDA tipoMon ON a.ID_TIPO_MONEDA = tipoMon.ID_TIPO_MONEDA 
	JOIN BI_SYSTEAM.BI_ESTADO_ANUNCIO estA ON a.ID_ESTADO_ANUNCIO = estA.ID_ESTADO_ANUNCIO
	where estA.ESTADO_ANUNCIO_DESCRIPCION IN ('Alquilado','Vendido')
  GROUP BY
    su.SUCURSAL_NOMBRE,
	T1.TIEMPO_ANIO,
	T1.TIEMPO_CUATRIMESTRE,
...
/*
-- Drop Vistas if they exist
IF OBJECT_ID('BI_SYSTEAM.VISTA_PROMEDIO_DIAS_PUBLICADOS_ANUNCIOS', 'V') IS NOT NULL
    DROP VIEW BI_SYSTEAM.VISTA_PROMEDIO_DIAS_PUBLICADOS_ANUNCIOS;

IF OBJECT_ID('BI_SYSTEAM.VISTA_PROMEDIO_ANUNCIOS_INMUEBLES', 'V') IS NOT NULL
    DROP VIEW BI_SYSTEAM.VISTA_PROMEDIO_ANUNCIOS_INMUEBLES;

IF OBJECT_ID('BI_SYSTEAM.VISTA_LOS_5_BARRIOS_MAS_ELEGIDOS_PARA_ALQUILAR', 'V') IS NOT NULL
    DROP VIEW BI_SYSTEAM.VISTA_LOS_5_BARRIOS_MAS_ELEGIDOS_PARA_ALQUILAR;

IF OBJECT_ID('BI_SYSTEAM.VISTA_PORCENTAJE_INCUMPLIMIENTO_PAGO_ALQUILER', 'V') IS NOT NULL
    DROP VIEW BI_SYSTEAM.VISTA_PORCENTAJE_INCUMPLIMIENTO_PAGO_ALQUILER;

IF OBJECT_ID('BI_SYSTEAM.VISTA_PORCENTAJE_PROMEDIO_INCREMENTO_VALOR_ALQUILERES', 'V') IS NOT NULL
    DROP VIEW BI_SYSTEAM.VISTA_PORCENTAJE_PROMEDIO_INCREMENTO_VALOR_ALQUILERES;

IF OBJECT_ID('BI_SYSTEAM.VISTA_PRECIO_PROMEDIO_M2_VENTA', 'V') IS NOT NULL
    DROP VIEW BI_SYSTEAM.VISTA_PRECIO_PROMEDIO_M2_VENTA;

IF OBJECT_ID('BI_SYSTEAM.VISTA_VALOR_PROMEDIO_COMISION', 'V') IS NOT NULL
    DROP VIEW BI_SYSTEAM.VISTA_VALOR_PROMEDIO_COMISION;

IF OBJECT_ID('BI_SYSTEAM.VISTA_PORCENTAJE_OPERACIONES_CONCRETADAS', 'V') IS NOT NULL
    DROP VIEW BI_SYSTEAM.VISTA_PORCENTAJE_OPERACIONES_CONCRETADAS;

IF OBJECT_ID('BI_SYSTEAM.VISTA_MONTO_TOTAL_CIERRE_CONTRATOS', 'V') IS NOT NULL
    DROP VIEW BI_SYSTEAM.VISTA_MONTO_TOTAL_CIERRE_CONTRATOS;
